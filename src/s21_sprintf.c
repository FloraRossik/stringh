...
char *get_decimal_str(long long int a, int is_neg, struct PutSettings settings) {
    long long int tmp = a;
    short num_len = 0;
    while (tmp) {
        tmp /= 10;
...
char *get_octa_str(long long a, struct PutSettings settings) {
    ...
    *ret_str = '\0';
    ...
    reverse(ret_str);
    ret_str = specify_int(ret_str, settings);

    return ret_str;
}

char *itoa(long long a, struct PutSettings settings) {
    char *ret_str = NULL;
    short is_neg = 0;
    if (a < 0) {
        is_neg = 1;
    ...
    if (settings.base == 10) {
        ret_str = get_decimal_str(a, is_neg, settings);
    } else if (settings.base == 16) {
        ret_str = get_hex_str(a, settings);
    } else if (settings.base == 8) {
        ret_str = get_octa_str(a, settings);
    }
...
const char *set_settings(const char *format, struct PutSettings *settings, va_list args, int set_def) {
    ...
    switch (*format) {
        case '-':
            ...
            if (*format == '*' || *format == '+' || *format == '#' || *format == ' ') {
                format = set_settings(format, settings, args, 0);
            }
            break;
        case '+':
    ...
    if (*format >= 48 && *format <= 57 && !settings->justify_num && settings->dot_specifier == -1) {
        settings->justify_num = get_num_from_format((char *)format);
    ...
    if (settings->dot_specified && settings->dot_specifier == -1) {
        if (*format == '*') {
            ...
            settings->dot_specifier = get_num_from_format((char *)format);
        }
    }
...
char *get_float_as_str(long double f_num, struct PutSettings settings) {
    ...
    set_default(&tmp_settings);
    char *int_str = itoa(int_part, settings);
    char *float_str = itoa(float_part, tmp_settings);
    float_str = specify_int(float_str, settings);
...
char *get_g_as_string(long double f_num, struct PutSettings settings) {
    ...
    *ret_str = '\0';
    ...
    if (settings.dot_specifier < int_len && !dot_fixed) {
        ...
        free(ret_str);
        ret_str = get_exp_as_str(tmp_l, settings);
        int exp = 0;
...
char *get_exponent_as_str(int exp) {
    ...
    ret_str = itoa(exp, tmp_settings);
    ret_str = specify_int(ret_str, tmp_settings);
...
char *get_exp_as_str(long double f_num, struct PutSettings settings) {
    ...
    if (settings.g_specified) {
        ...
        mantissa = get_float_as_str(f_num, settings);
    }
    ...
    char *exponent = get_exponent_as_str(exp);
...
char *get_arg_as_str(const char *format, struct PutSettings *settings, va_list args) {
    ...
    switch (*format) {
        case 'c':
            ...
            *(ret_str + 1) = '\0';
            ret_str = justify_flags(ret_str, *settings);
            break;
        ...
        case 'f':
            ...
            ret_str = get_float_as_str(f_num, *settings);
            ret_str = justify_flags(ret_str, *settings);
            break;
        case 'E':
        case 'e':
            ...
            ret_str = get_exp_as_str(f_num, *settings);
            ret_str = justify_flags(ret_str, *settings);
            break;
        case 'X':
        case 'x':
...
int s21_sprintf(char *str, const char *format, ...) {
    ...
    for (; *format; format++) {
        if (*format == '%') {
            ...
            format = set_settings(format, &settings, args, 1);
            src = get_arg_as_str(format, &settings, args);
            *str = '\0';
            s21_strcat(str, src);
            str += s21_strlen(src);
